...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!requiresNewPassword) {
        // After login, checkRole will be called in AuthContext
        // Wait a moment for role check to complete, then navigate
        setTimeout(() => {
          // Navigate - ProtectedRoute will handle redirect if not admin
          navigate('/admin')
        }, 200)
      }
    } catch (err: any) {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After password change, checkRole will be called in AuthContext
      // Wait a moment for role check to complete, then navigate
      setTimeout(() => {
        navigate('/admin')
      }, 200)
